public class Study_Java61 {
		/*
		 ★ 메소드 오버로딩(OverLoading)
		 	- 클래스 안에 같은 이름을 가지고 있는 메소드를 여러개 선언(정의)
		 	- 매개변수의 개수가 달라야 한다.
		 	- 매개변수의 개수가 같을 경우에는 매개변수 데이터 타입이 달라야 한다. 
		 */
	public static void main(String[] args) {	
		...
		System.out.println("=============");
		getSum(10, 10.5);	//메소드를 만들진 않았지만 자동으로 형변환되어 getSum(double, double)메소드 호출
	}//main()
	...
	public static void getSum(int x, int y, int z) {	//getSum(int, int, int) 메소드 정의
		System.out.println("첫 번째 인수: "+x);
		System.out.println("두 번째 인수: "+y);
